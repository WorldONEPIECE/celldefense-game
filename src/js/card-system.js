/**
 * card-system.js — 卡牌系统
 * T16: loadCards() 移除 pool_excluded_phase1 过滤，40张卡全部进池。
 */
...
export class CardSystem {
  constructor(cardsData, balanceConfig, resources, effectResolver, gameState, i18n) {
    this._allCards=cardsData; this._balance=balanceConfig; this._resources=resources;
  ...
  buildStarterDeck() {
    ...
    for (const id of starterIds) {
      ...
      else console.warn(`[CardSystem] Starter card not in pool: "${id}"`);
    }
    this._gameState.deck=this.shuffle(deck); this._gameState.discard=[]; this._gameState.hand=[];
  ...
  draw(n) {
    for (let i=0; i<n; i++) {
      if (this._gameState.deck.length===0) {
        ...
        this._gameState.deck=this.shuffle(this._gameState.discard); this._gameState.discard=[];
  ...
  playCard(cardId) {
    ...
    if (idx===-1) { console.warn(`[CardSystem] Card not in hand: "${cardId}"`); return false; }
    const card=this._gameState.hand[idx];
    if (card.play_condition && !this._effectResolver._evaluateCondition(card.play_condition)) { console.log(`[CardSystem] Play condition not met: "${cardId}"`); return false; }
    if (!this._resources.canAfford(card.cost||{})) { console.log(`[CardSystem] Cannot afford: "${cardId}"`); return false; }
    this._resources.pay(card.cost||{});
    const nullified = this._gameState.isCardNullified?.(cardId) ?? false;
    if (!nullified) this._effectResolver.resolve(card.effects||[], { source:'player', card });
    else console.log(`[CardSystem] Effect nullified by virus: "${cardId}"`);
  ...
  discardHand() {
    for (const card of this._gameState.hand) { this._gameState.discard.push(card); EventBus.emit(EVENTS.CARD_DISCARDED, { cardId:card.id }); }
    this._gameState.hand=[];
    this._notifyHandUpdate();
  }

  getStats() { return { deckSize:this._gameState.deck.length, discardSize:this._gameState.discard.length, handSize:this._gameState.hand.length }; }
