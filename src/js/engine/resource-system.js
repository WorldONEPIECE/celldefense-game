/**
 * resource-system.js — 注册表式资源系统
 *
 * v1.1 T9：has() 方法；canAfford() 加 has() 保护
 *          移除 processTurnRegen() 中重复的 VIRAL_LOAD_OVERLOAD emit
 *          （该事件由 VirusAI.processTurn() 统一管理，只在过载状态转变时触发）
 */
...
export class ResourceSystem {
  ...
  init() {
    const defs      = this._config.resources;
    ...
    for (const [id, def] of Object.entries(defs)) {
      ...
      this._resources.set(id, {
        id, current: base, base, max, regen,
        display_key: def.display_key,
        color:       def.color,
        icon:        def.icon,
  ...
  processTurnRegen() {
    ...
    if (fatigue >= g.fatigue_penalty_tier2) {
      fatiguePenalty = g.fatigue_resource_penalty_per_tier * 2;
    } else if (fatigue >= g.fatigue_penalty_tier1) {
      fatiguePenalty = g.fatigue_resource_penalty_per_tier;
    }
    for (const [id, state] of this._resources) {
      if (state.regen === 0) continue;
      let regenAmount = state.regen;
      if (regenAmount > 0 && !state.is_negative) {
        regenAmount = Math.max(0, regenAmount - fatiguePenalty);
      }
    ...
    // 注意：不在此处 emit VIRAL_LOAD_OVERLOAD，由 VirusAI 统一管理
  }
  ...
  canAfford(cost) {
    for (const [id, amount] of Object.entries(cost)) {
      if (!this.has(id) || this.get(id) < amount) return false;
    }
    return true;
  }

  pay(cost) {
    for (const [id, amount] of Object.entries(cost)) this.delta(id, -amount);
  }
  ...
  _getState(id) {
    const state = this._resources.get(id);
    if (!state) throw new Error(`[ResourceSystem] Unknown resource: "${id}"`);
    return state;
  }
  ...
  _emitChange(id, prev, current) {
    ...
    if (current <= 0 && id === 'cell_integrity') {
      EventBus.emit(EVENTS.GAME_OVER, { reason: 'cell_integrity_depleted' });
    }
    if (current <= 0 && prev > 0) {
      EventBus.emit(EVENTS.RESOURCE_DEPLETED, { id });
    }
